...
export class OrderController {
  ...
  static async getOrderByToken(req: Request, res: Response) {
    try {
  ...
  static async updateOrderStatusByToken(req: Request, res: Response) {
    try {
      const { token } = req.params;
      ...
      const order = await OrderService.updateOrderStatusByToken(token, status);
      return res.status(200).json({ success: true, data: order });
    } catch (err: any) {
